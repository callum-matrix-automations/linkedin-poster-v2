...
const FILTER_SAMPLING_PARAMS = CONFIG.filter_sampling_params === true;
const FALLBACK_TO_CLAUDE_CODE = CONFIG.fallback_to_claude_code !== false;
  ...
  async loadFromClaudeCodeCredentials() {
    try {
      ...
      if (oauth.expiresAt && Date.now() >= (oauth.expiresAt - 10000)) {
        ...
        return await this.refreshTokenWithOauth();
      }
    ...
    try {
      ...
      if (error.code === 'ENOENT') {
        const errorMsg = process.platform === 'win32'
          ? 'Failed to load credentials: Claude credentials file not found. Check that Claude Code is installed or use OAuth authentication instead.'
          : 'Claude credentials not found. Please ensure Claude Code is installed and you have logged in, or use OAuth authentication instead.';
