...
async function handleRequest(req, res) {
  ...
  // API key authentication (skip for auth routes, static, and health check)
  const apiKey = process.env.API_KEY;
  const isPublicRoute = pathname.startsWith('/auth/') || pathname === '/health' || pathname === '/';
  ...
  if (req.method === 'GET' && pathname === '/v1/models') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify(OpenAITranslator.getModelsResponse()));
    return;
  }

  // OpenAI-compatible: POST /v1/chat/completions (with optional preset)
  if (req.method === 'POST' && (pathname === '/v1/chat/completions' || pathname.match(/^\/v1\/\w+\/chat\/completions$/))) {
    try {
      Logger.debug('Incoming OpenAI-compat request headers:', JSON.stringify(req.headers, null, 2));
      const body = await parseBody(req);
      Logger.debug(`OpenAI request body (${JSON.stringify(body).length} bytes):`, JSON.stringify(body, null, 2));

      let presetName = null;
      const presetMatch = pathname.match(/^\/v1\/(\w+)\/chat\/completions$/);
      if (presetMatch) {
      ...
      const requestId = OpenAITranslator.generateRequestId();

      const anthropicBody = OpenAITranslator.translateRequest(body);
      Logger.debug('Translated to Anthropic body:', JSON.stringify(anthropicBody, null, 2));

      const claudeReq = new ClaudeRequest(req);
      let response = await claudeReq.makeRequest(anthropicBody, presetName);

      if (response.statusCode === 401) {
        Logger.info('Got 401 on OpenAI-compat request, refreshing token');
        ClaudeRequest.cachedToken = null;
        try {
          const newToken = await claudeReq.loadOrRefreshToken();
          ClaudeRequest.cachedToken = newToken;
          response = await claudeReq.makeRequest(anthropicBody, presetName);
        } catch (refreshError) {
          Logger.info('Token refresh failed, returning 401');
        }
      }
      ...
      if (response.statusCode !== 200) {
        ...
        response.on('end', () => {
          Logger.debug('Anthropic error response:', errorBody);
          res.writeHead(response.statusCode, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({
            error: {
              message: `Upstream API error: ${response.statusCode}`,
              type: 'api_error',
      ...
      if (isStreaming && contentType.includes('text/event-stream')) {
        ...
        const transform = OpenAITranslator.createStreamTransform(requestId, requestModel);

        response.on('error', (err) => {
          Logger.debug('Claude SSE stream error:', err);
          if (!res.destroyed) res.end('data: [DONE]\n\n');
        });

        res.on('close', () => {
          Logger.debug('Client disconnected from OpenAI-compat stream');
          if (!response.destroyed) response.destroy();
        });

        transform.on('error', (err) => {
          Logger.debug('OpenAI transform stream error:', err);
          if (!res.destroyed) res.end('data: [DONE]\n\n');
        });
        ...
        if (Logger.getLogLevel() >= 3) {
          ...
          const debugStream = Logger.createDebugStream('Claude SSE (OpenAI-compat)', extractClaudeText);
          response.pipe(debugStream).pipe(transform).pipe(res);
        } else {
        ...
        response.on('end', () => {
          try {
            const anthropicJson = JSON.parse(responseData);
            Logger.debug('Anthropic response:', JSON.stringify(anthropicJson, null, 2));
            const openaiJson = OpenAITranslator.translateResponse(anthropicJson, requestModel);
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify(openaiJson));
            Logger.debug('OpenAI-compat non-streaming response sent');
          } catch (e) {
            Logger.error('Failed to translate Anthropic response:', e.message);
            res.writeHead(500, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: { message: 'Failed to process response', type: 'server_error' } }));
          }
        });
        response.on('error', (err) => {
          Logger.error('Anthropic response error:', err);
          if (!res.headersSent) res.writeHead(502, { 'Content-Type': 'application/json' });
          if (!res.destroyed) res.end(JSON.stringify({ error: { message: 'Upstream error', type: 'server_error' } }));
        });
      }
    } catch (error) {
      Logger.error('OpenAI-compat request error:', error.message);
      res.writeHead(500, { 'Content-Type': 'application/json' });
